			...
			for (String rule : spCommand.getRules()) {
				...
				try {
					...
					String message = e.getLocalizedMessage();
						...
						int parsedLine = Integer.parseInt(message.substring(
								lineIndex, columnWordIndex - 2));
						int parsedColumn = Integer.parseInt(message.substring(
								columnIndex, periodIndex));
					...
					/**
					 * Get the range of the error, based on the string being
					 * parsed and the given column and row
					 */
					/*
					 * int start = 0; for (int i = 1; i < e.getErrorLine();) {
					 * char c = rule.charAt(start); if (c == '\n') { ++i; }
					 * ++start; }
					 */
					/*
					 * int start = 0; for (int i = 1; i < e.getErrorLine();) {
					 * char c = parseText.charAt(start); if (c == '\n') { ++i; }
					 * ++start; }
					 * 
					 * start += beginIndex; // -1 for columns counting starting
					 * with 1 start += e.getErrorColumn() - 1;
					 * 
